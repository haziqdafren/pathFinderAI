...
import { googleSignInForGmail, sendGmailMessage, auth, isGmailApiConfigured } from '../utils/gmailAuth';
  ...
  const handleSendEmail = async () => {
    ...
    const confirmed = window.confirm(
      isGmailApiConfigured
        ...
        : `PathFinder akan membuka Gmail Compose untuk ${recipient}. Kamu tetap perlu klik Send di Gmail. Lanjut?`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      toast.success(isGmailApiConfigured ? `Laporan kesiapan karir berhasil dikirim ke ${recipient}!` : 'Gmail compose dibuka. Periksa isi email lalu klik Send di Gmail.');
      onClose();
    } catch (err) {
      console.error(err);
          ...
          <div className="text-center py-5">
            ...
            <p className="text-[13.5px] text-muted-dark leading-relaxed mb-6 max-w-[340px] mx-auto">
              {isGmailApiConfigured
                ...
                : 'Konfigurasi Firebase Gmail belum aktif, jadi PathFinder akan membuka Gmail Compose dengan isi laporan yang sudah disiapkan.'}
            </p>
            ...
            >
              {isLoggingIn ? (
                ...
                <>
                  ...
                  <span>{isGmailApiConfigured ? 'Hubungkan via Gmail' : 'Buka Gmail Compose'}</span>
                </>
              )}
            </button>
          </div>
